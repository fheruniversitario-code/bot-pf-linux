...
function aplicarFiltroCitas() {
    ...
    filtradas.forEach(c => {
        ...
        tr.innerHTML = `
            ...
            <td class="py-3 px-1">
                <div class="text-sm font-semibold text-slate-200">${c.cliente_nombre || 'Sin Nombre'}</div>
                <div class="text-[10px] text-slate-400 italic">${c.notas || ''}</div>
            </td>
            <td class="py-3 px-1 text-sm text-slate-300">
                <div class="flex items-center space-x-2">
                    <span>${c.cliente_telefono || '-'}</span>
            ...
            <td class="py-3 px-1 text-sm text-slate-300">${c.servicio || '-'}</td>
            <td class="py-3 px-1 text-xs">
                <div class="flex items-center space-x-1.5 text-slate-400 bg-slate-800/50 px-2 py-1 rounded w-max">
                    <i class="fa-solid fa-desktop text-[10px]"></i><span>Panel Web</span>
                </div>
...
async function cargarConfigRecordatorios() {
    try {
        ...
        if (conf) {
            ...
            document.getElementById('config-recordatorio-texto').value = conf.recordatorios_texto || 'Hola {nombre} ??, te recordamos tu cita programada para hoy a las {hora} para {servicio}. �Nos confirmas tu asistencia? (Si deseas cancelar o reagendar, por favor av�sanos por este medio).';
        }
    } catch (e) {
...
async function guardarConfigRecordatorios() {
    ...
    try {
        ...
        Swal.fire('Guardado', 'La configuraci�n de recordatorios ha sido guardada con �xito.', 'success');
    } catch (e) {
