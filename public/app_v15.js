...
async function abrirModalAgendarChat() {
    try {
        ...
        if (!res.disponibles || res.disponibles.length === 0) {
            Swal.fire('Sin horarios', 'No hay horarios disponibles para el ' + fecha, 'info');
            return;
        }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const telLimpio = chatActivoJid.replace(/[^0-9]/g, '');
                let nombre = 'Paciente';
                const contactoItem = document.querySelector(`.chat-item[onclick*="${chatActivoJid}"]`);
                if (contactoItem) {
