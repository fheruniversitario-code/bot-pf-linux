...
function renderizarListaConversaciones(chats) {
    ...
    chats.forEach(c => {
        ...
        let bestPhone = c.telefono;
        if (c.jid && c.jid.includes('@lid') || (c.telefono && c.telefono.length > 13)) {
            // Si el telefono oficial es un LID, usamos el pushname si contiene numeros
            if (c.pushname && c.pushname.replace(/[^0-9]/g, '').length >= 10) {
                bestPhone = c.pushname.replace(/[^0-9]/g, '');
            }
        }
        
        div.onclick = () => seleccionarChat(c.jid, c.nombre || c.pushname || c.telefono, bestPhone);
...
async function seleccionarChat(jid, nombre, telefono = '') {
    ...
    window.chatActivoTelefono = telMostrar || jid.replace('@s.whatsapp.net', '').split('@')[0];
...
async function cargarAgendaCitas() {
    try {
        ...
        const filtroFecha = document.getElementById('filtro-fecha-input');
        if (filtroFecha && !filtroFecha.dataset.initialized) {
            const _now = new Date(); filtroFecha.value = new Date(_now.getTime() - (_now.getTimezoneOffset() * 60000)).toISOString().split('T')[0];
            filtroFecha.dataset.initialized = 'true';
        }
        aplicarFiltroCitas();
...
async function guardarNuevaCita(e) {
    e.preventDefault();
    try {
        const cliente_nombre = document.getElementById('cita-nombre-input').value.trim().toUpperCase();
        const cliente_telefono = document.getElementById('cita-telefono-input').value.trim();
        const servicio = document.getElementById('cita-servicio-input').value.trim().toUpperCase();
        ...
        const exp = document.getElementById('cita-expediente-input')?.value.trim().toUpperCase();
        if (exp) {
            notas = (notas ? notas + '\n' : '') + 'Expediente: ' + exp;
        }
        ...
        if (window._citaDesdeChat && window._citaChatJid) {
            ...
            fetch(`/api/conversaciones/${encodeURIComponent(window._citaChatJid)}/enviar`, {
                ...
                body: JSON.stringify({ texto: msjConfirmacion, esAgente: true })
            });
            window._citaDesdeChat = false;
            window._citaChatJid = null;
        }
    } catch (err) {
...
async function cargarLinktreeConfig() {
    try {
        ...
        if (data.logo_url && document.getElementById('header-logo-container')) {
            document.getElementById('header-logo-container').innerHTML = '<img src="' + data.logo_url + '" class="w-full h-full object-cover rounded-xl">';
        }
...
async function abrirModalAgendarChat() {
    window.citaOrigenJid = chatActivoJid;
    if (!chatActivoJid) return Swal.fire('Error', 'Selecciona un chat primero.', 'error');
    
    // Obtener los datos del chat activo
    const nombreHeader = document.getElementById('chat-nombre-cliente').textContent.trim();
    const onlyDigitsHeader = nombreHeader.replace(/[^0-9]/g, '');
    
    let telefono = window.chatActivoTelefono || chatActivoJid.replace('@s.whatsapp.net', '').split('@')[0];
    
    // Si el telefono detectado es muy largo (probablemente un LID) o es @lid, intentamos sacar el real del nombre
    if (telefono.length > 13 || chatActivoJid.includes('@lid')) {
        if (onlyDigitsHeader.length >= 10 && onlyDigitsHeader.length <= 15) {
            telefono = onlyDigitsHeader;
        } else {
            // Escanear los �ltimos mensajes del DOM buscando un tel�fono proporcionado por el paciente
            let foundPhone = '';
            const msgStream = document.getElementById('chat-mensajes-stream');
            if (msgStream) {
                // Seleccionamos los textos de los mensajes
                const textNodes = msgStream.querySelectorAll('p.whitespace-pre-wrap, div.whitespace-pre-wrap');
                // Recorremos de abajo hacia arriba (los m�s recientes primero)
                for (let i = textNodes.length - 1; i >= 0; i--) {
                    const txt = textNodes[i].textContent;
                    // Busca secuencias de n�meros que pueden tener espacios o guiones intermedios
                    const match = txt.match(/(?:\+?\d[\d\s\-\.]{8,15}\d)/g);
                    if (match) {
            ...
            telefono = foundPhone || ''; // Si encontr� uno, lo usa. Si no, lo deja limpio.
        }
    }
    
    let nombre = nombreHeader;
    // Si el nombre es igual al telefono (con o sin +), o es gen�rico, lo dejamos en blanco
    if (nombre === 'Cliente' || nombre.replace(/[^0-9]/g, '') === telefono) {
        nombre = ''; 
    }

    const dateHoy = new Date();
    dateHoy.setMinutes(dateHoy.getMinutes() - dateHoy.getTimezoneOffset());
    const hoyStr = dateHoy.toISOString().split('T')[0];
    
    // Rellenar el modal estandar de citas
    if(document.getElementById('cita-id-input')) document.getElementById('cita-id-input').value = '';
    document.getElementById('cita-nombre-input').value = nombre;
    document.getElementById('cita-expediente-input').value = '';
    document.getElementById('cita-telefono-input').value = telefono;
    document.getElementById('cita-servicio-input').value = '';
    document.getElementById('cita-fecha-input').value = hoyStr;
    document.getElementById('cita-hora-input').value = '10:00';
    document.getElementById('cita-duracion-input').value = window._ultimaDuracion || '30';
    document.getElementById('cita-notas-input').value = 'Agendado desde Chat';
    
    window._citaDesdeChat = true;
    window._citaChatJid = chatActivoJid;
...
function imprimirReporteCitas() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed && result.value) {
            ...
            if (typeof todasLasCitas === 'undefined' || todasLasCitas.length === 0) {
                return Swal.fire('Error', 'No hay citas cargadas en el sistema.', 'error');
            }
            const citasDelDia = todasLasCitas.filter(c => c.fecha === date && c.estado !== 'Cancelada');
            ...
            <h2>Reporte de Citas / Archivo Clinico</h2>
...
function aplicarFiltroCitas() {
    const term = (document.getElementById('buscar-cita-input')?.value || '').toLowerCase().trim();
    const filterDate = document.getElementById('filtro-fecha-input')?.value || '';
    if (!todasLasCitas) return;
    
    let filtradas = todasLasCitas;
    
    if (filterDate) {
        filtradas = filtradas.filter(c => c.fecha === filterDate);
    }
    ...
    if (term) {
        filtradas = filtradas.filter(c => 
            ...
            (c.servicio && c.servicio.toLowerCase().includes(term)) ||
            (c.notas && c.notas.toLowerCase().includes(term)) ||
            (c.cliente_telefono && c.cliente_telefono.includes(term))
        );
    }
    ...
    if (filtradas.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="py-8 text-center text-slate-500 text-xs">No se encontraron citas.</td></tr>';
        return;
    }
    
    filtradas.forEach(c => {
        const _today = new Date(); const todayIso = new Date(_today.getTime() - (_today.getTimezoneOffset() * 60000)).toISOString().split('T')[0]; const esHoy = c.fecha === todayIso;
        ...
        tr.innerHTML = `
            ...
            <td class="py-3 px-1">
                <div class="text-sm font-semibold text-slate-200" style="text-transform: uppercase;">${c.cliente_nombre || 'Sin Nombre'}</div>
                <div class="text-[10px] text-slate-400 italic" style="text-transform: uppercase;">${c.notas || ''}</div>
            </td>
            ...
            <td class="py-3 px-1 text-xs">
                <div class="flex items-center space-x-1.5 text-slate-400 bg-slate-800/50 px-2 py-1 rounded w-max">
                    <i class="fa-solid fa-desktop text-[10px]"></i><span>Panel Web</span>
                </div>
                ${c.id_evento_calendar ? '<div class="flex items-center space-x-1 mt-1 text-emerald-400 text-[10px] font-bold"><i class="fa-brands fa-google"></i><span>Sincronizado</span></div>' : ''}
            </td>
            <td class="py-3 px-1 text-xs">
...
async function cargarConfigRecordatorios() {
    try {
        const conf = await apiFetch('/api/configuracion');
        if (conf) {
            document.getElementById('config-recordatorio-activo').checked = (conf.recordatorios_activo === '1');
            document.getElementById('config-recordatorio-hora').value = conf.recordatorios_hora || '08:00';
            document.getElementById('config-recordatorio-texto').value = conf.recordatorios_texto || 'Hola {nombre} 👋, te recordamos tu cita programada para hoy a las {hora} para {servicio}. ¿Nos confirmas tu asistencia? (Si deseas cancelar o reagendar, por favor avísanos por este medio).';
        }
    } catch (e) {
...
async function guardarConfigRecordatorios() {
    const activo = document.getElementById('config-recordatorio-activo').checked ? '1' : '0';
    const hora = document.getElementById('config-recordatorio-hora').value;
    const texto = document.getElementById('config-recordatorio-texto').value.trim();

    if (!hora || !texto) {
        return Swal.fire('Error', 'Debes definir una hora y el texto del mensaje.', 'error');
    }
    ...
    try {
        await apiFetch('/api/configuracion', {
            method: 'POST',
            body: JSON.stringify({ 
                recordatorios_activo: activo,
                recordatorios_hora: hora,
                recordatorios_texto: texto
            })
        });
        Swal.fire('Guardado', 'La configuración de recordatorios ha sido guardada con éxito.', 'success');
    } catch (e) {
...
async function sincronizarConGoogle() {
    try {
        const icon = document.querySelector('button[onclick="sincronizarConGoogle()"] i');
