...
client.on('message_create', async (msg) => {
    try {
        ...
        // ──        // 🔴 CHECK #0: Esperar 3000ms para dar tiempo a que los sendMessage registren sus IDs y evitar falsos positivos por latencia de red
        // Removida la lógica restrictiva de botEnviosPendientes y jidBotActivo que causaba que mensajes humanos concurrentes fueran ignorados.
        ...
        if (!targetJid || targetJid === 'status@broadcast' || targetJid.endsWith('@g.us')) return;
...
// Ejecutar worker cada 15 minutos
setInterval(procesarSeguimientosAutomaticos, 60 * 1000); // Revisin precisa cada minuto


// --------------------------------------------------------------------------
// CRON: RECORDATORIOS AUTOMTICOS DE CITAS
// --------------------------------------------------------------------------
let ultimoMinutoRecordatorio = -1;
...
setInterval(async () => {
    try {
        ...
        if (citasDeHoy && citasDeHoy.length > 0) {
            console.log(`[CRON RECORDATORIOS] Iniciando envo de ${citasDeHoy.length} recordatorios...`);
            for (const cita of citasDeHoy) {
                // Formatear JID
                let jid = cita.cliente_telefono.replace(/\D/g, '');
                if (jid.length === 10) jid = '521' + jid; // Default a Mxico celular (o 52 sin 1)
                if (!jid.includes('@s.whatsapp.net')) jid += '@s.whatsapp.net';
                ...
                if (client && client.user) {
                    ...
                    // Pequea pausa para no saturar WhatsApp
                    await new Promise(r => setTimeout(r, 2000));
                }
            }
        }
...
client.on('message_create', async (msg) => {
    try {
        ...
        // Si no existe el contacto, crearlo para que aparezca en la lista del panel
        await runQuery(
            `INSERT INTO contactos (jid, telefono, nombre, pushname, ultimo_contacto) VALUES (?, ?, 'Nuevo Paciente', '', ?) ON CONFLICT(jid) DO UPDATE SET ultimo_contacto = ?`,
            [targetJid, telClean, Date.now(), Date.now()]
        );
