...
const idsMensajesRecibidos = new Set(); // Para deduplicaciÃ³n estricta de mensajes entrantes
const chatsEnProceso = new Map(); // JID -> Timestamp inicio (bloqueo concurrente con expiraciÃ³n automÃ¡tica TTL)
const ultimosTextosEnviadosBot = new Map(); // Texto limpio -> Timestamp (para evitar que el bot se auto-pause a sÃ­ mismo)
const colasProcesamiento = new Map();

const ultimosJidsEnviadosBot = new Map(); // JID / Ãºltimos 8 dÃ­gitos -> Timestamp
...
// ------------------------------------------------------------------------------
// 1. CONFIGURACIÃ“N DE GEMINI AI
// ------------------------------------------------------------------------------
const geminiApiKey = process.env.GEMINI_API_KEY || process.env.API_KEY || '';
const genAI = geminiApiKey ? new GoogleGenerativeAI(geminiApiKey) : null;
// Modelos se detectan dinÃ¡micamente via obtenerModelosDisponibles() â€” no hardcodear aquÃ­
...
function autenticarToken(req, res, next) {
    ...
    jwt.verify(token, JWT_SECRET, (err, user) => {
        if (err) return res.status(403).json({ error: 'SesiÃ³n invÃ¡lida o expirada' });
        req.user = user;
        next();
    });
}
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        if (!passValida) return res.status(400).json({ error: 'Usuario o contraseÃ±a incorrectos' });
...
// Cambio de ContraseÃ±a y Datos de Perfil (Para el Cliente)
app.post('/api/perfil/cambiar-password', autenticarToken, async (req, res) => {
    try {
        ...
        if (password_actual && nuevo_password) {
            ...
            if (!passValida) return res.status(400).json({ error: 'La contraseÃ±a actual no es correcta' });
...
// EstadÃ­sticas y MÃ©tricas (KPIs de Ventas y CRM)
...
function formatearTiempoRelativo(timestampMs) {
    ...
    else if (difDias === 1) texto = `hace 1 dÃ­a (${fechaFormateada})`;
    else if (difDias < 30) texto = `hace ${difDias} dÃ­as (${fechaFormateada})`;
    else if (difMeses === 1) texto = `hace 1 mes (${fechaFormateada})`;
    else if (difMeses < 12) texto = `hace ${difMeses} meses (${fechaFormateada})`;
    else texto = `hace mÃ¡s de 1 aÃ±o (${fechaFormateada})`;
...
app.post('/api/directorio', autenticarToken, async (req, res) => {
    try {
        ...
        if (!telefono) return res.status(400).json({ error: "El telÃ©fono es requerido" });
    ...
    try {
        ...
        const chats = await allQuery(`
            SELECT c.jid, c.telefono, c.nombre, c.pushname, c.correo, c.expediente, c.domicilio, c.es_ignorado, c.ultimo_contacto,
                   (SELECT CASE 
                        WHEN cuerpo LIKE '/9j/%' OR cuerpo LIKE 'data:image%' THEN 'ðŸ“· (Imagen / InfografÃ­a)'
                        ELSE cuerpo 
...
// ------------------------------------------------------------------------------
// GESTIÃ“N DE ETIQUETAS / LISTAS DE WHATSAPP (LABELS & CRM TAGS)
// ------------------------------------------------------------------------------
...
app.post('/api/contactos/:jid/etiquetas', autenticarToken, async (req, res) => {
    try {
        ...
        // SincronizaciÃ³n hacia WhatsApp Business en vivo (si la cuenta vinculada es Business)
...
// Sincronizar / Importar Etiquetas y Asignaciones desde WhatsApp Business (ExtracciÃ³n Directa)
app.post('/api/etiquetas/sincronizar-whatsapp', autenticarToken, async (req, res) => {
    try {
        if (!client || !wsClienteConectado) {
            return res.status(400).json({ error: 'El bot no estÃ¡ conectado a WhatsApp en este momento. AsegÃºrate de que el bot estÃ© en verde y conectado.' });
        }
        ...
        try {
            ...
            return res.status(400).json({ 
                error: 'Error al consultar WhatsApp Web. AsegÃºrate de que el bot estÃ© conectado a una cuenta de WhatsApp Business. Detalle: ' + errEval.message 
            });
        }
        ...
        for (const ch of dataWA.chats) {
            ...
            // Solo considerar tReal si el chat realmente tiene mensajes vÃ¡lidos
            ...
            // Guardar Ãºltimo mensaje para vista previa en el panel si no existe y no es notificaciÃ³n
        ...
        res.json({
            success: true,
            message: `Â¡SincronizaciÃ³n completada! Se importaron ${dataWA.labels.length} etiquetas reales y se vincularon ${asignacionesTotal} clientes con sus nombres y fechas histÃ³ricas.`,
...
// Bandeja de clientes/Clientes con Seguimiento Pendiente o PrÃ³ximo
    ...
    try {
        ...
        for (const r of reglas) {
            ...
            for (const c of contactosCandidatos) {
                ...
                // Verificar si ya se enviÃ³ este seguimiento
...
app.post('/api/seguimientos/enviar', autenticarToken, async (req, res) => {
    try {
        ...
        if (!jid || !mensaje || !client) return res.status(400).json({ error: 'Faltan parÃ¡metros o WhatsApp no estÃ¡ conectado' });
        ...
        await runQuery(
            ...
            [jid, 'bot', 'Seguimiento AutomÃ¡tico', mensaje, Date.now()]
        );

        io.emit('nuevo_mensaje', {
            chat_id: jid,
            emisor: 'bot',
            emisor_nombre: 'Seguimiento AutomÃ¡tico',
            cuerpo: mensaje,
            es_mio: 1,
            es_ia: 1,
            timestamp: Date.now()
        ...
        res.json({ success: true, message: 'Recordatorio de seguimiento enviado con Ã©xito' });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
});

// Mensajes de un Chat especÃ­fico (con sincronizaciÃ³n en vivo de WhatsApp y unificaciÃ³n de LID y telÃ©fono)
app.get('/api/conversaciones/:jid/mensajes', autenticarToken, async (req, res) => {
    try {
        ...
        let waChat = null;
        // Sincronizar mensajes recientes de WhatsApp Web para reflejar lo que envÃ­as desde el celular oficial
        if (client && wsClienteConectado) {
            try {
                ...
                // 2. Si no se encontrÃ³ (muy comÃºn con identificadores @lid), buscar vÃ­a contacto de WhatsApp
                if (!waChat) {
                    try {
                ...
                // 3. Si aÃºn no se encontrÃ³ y tenemos telÃ©fono limpio vÃ¡lido
                ...
                // 4. Si aÃºn no se encontrÃ³, buscar en los chats en memoria por coincidencia de nombre o nÃºmero
                if (!waChat) {
                    try {
                ...
                // Si encontramos el chat en WhatsApp Web, traer los Ãºltimos 60 mensajes
                let rawMsgs = [];
                if (waChat) {
                    try {
                        rawMsgs = await waChat.fetchMessages({ limit: 60 });
                    } catch(eFetch) {
                        console.warn("waChat.fetchMessages fallÃ³, intentando extracciÃ³n directa:", eFetch.message);
                    }
                }

                // Si rawMsgs estÃ¡ vacÃ­o o fallÃ³, extraer directamente desde Puppeteer en la memoria de WhatsApp Web
                ...
                if (rawMsgs && rawMsgs.length > 0) {
                    ...
                    for (const m of rawMsgs) {
                        ...
                        let cuerpoTxt = m.body || (m.hasMedia ? 'ðŸ“· (InfografÃ­a / Imagen enviada)' : (m.type === 'chat' ? '' : `ðŸ’¬ (${m.type || 'Mensaje'})`));
                        ...
                        if (cuerpoTxt.startsWith('/9j/') || cuerpoTxt.startsWith('data:image') || (cuerpoTxt.length > 200 && !cuerpoTxt.includes(' '))) {
                            cuerpoTxt = 'ðŸ“· (InfografÃ­a / Imagen enviada)';
                        }
                        ...
                        // Actualizar nombre si aÃºn no estaba personalizado
        ...
        // Obtener mensajes unificando por JID, chat vinculado y Ãºltimos 8 dÃ­gitos del telÃ©fono
        const ultimos8 = (telLimpio && !telLimpio.startsWith('1660') && telLimpio.length >= 8) ? telLimpio.slice(-8) : '';
        const chatVinculado = waChat?.id?._serialized || jid;
        ...
        // Deduplicar mensajes en memoria y normalizar multimedia/infografÃ­as
        const mensajes = [];
        ...
        for (const m of todosMensajes) {
            ...
            if (cuerpoNormalizado.startsWith('/9j/') || cuerpoNormalizado.startsWith('data:image') || (cuerpoNormalizado.length > 200 && !cuerpoNormalizado.includes(' '))) {
                cuerpoNormalizado = 'ðŸ“· (InfografÃ­a / Imagen enviada)';
                m.cuerpo = cuerpoNormalizado;
            } else if (cuerpoNormalizado === 'ðŸ“· (Multimedia enviado desde telÃ©fono)' || cuerpoNormalizado === 'ðŸ“· (Archivo multimedia)') {
                cuerpoNormalizado = 'ðŸ“· (InfografÃ­a / Imagen enviada)';
                m.cuerpo = cuerpoNormalizado;
            }
...
// Enviar Mensaje desde el Panel Web (IntervenciÃ³n Humana)
app.post('/api/conversaciones/:jid/enviar', autenticarToken, async (req, res) => {
    try {
        const { jid } = req.params;
        const { texto } = req.body;
        if (!texto || !client) return res.status(400).json({ error: 'Mensaje vacÃ­o o bot desconectado' });
        ...
        // Pausar automÃ¡ticamente el bot en este chat por 30 minutos
        chatsPausados.set(jid, Date.now());

        // Si el cliente tenÃ­a una solicitud de asesor pendiente, marcarla como atendida
...
// Reanudar Bot en un chat especÃ­fico (Quitar pausa de asesor humano y volver a IA)
app.post('/api/conversaciones/:jid/reactivar', autenticarToken, async (req, res) => {
    try {
        const jid = decodeURIComponent(req.params.jid);
        chatsPausados.delete(jid);

        // Limpiar tambiÃ©n jids y telÃ©fonos asociados
        const telClean = jid.replace(/[^0-9]/g, '');
        ...
        console.log(`ðŸ¤– [BOT REANUDADO] El bot volverÃ¡ a responder con IA en el chat ${jid}`);
...
// Enviar InfografÃ­a o Imagen desde el Panel Web por WhatsApp
app.post('/api/conversaciones/:jid/enviar-imagen', autenticarToken, uploadImagen.single('imagen'), async (req, res) => {
    try {
        ...
        if (!rutaArchivo || !fs.existsSync(rutaArchivo)) {
            return res.status(400).json({ error: 'No se encontrÃ³ la imagen especificada para enviar' });
        }

        if (!client || !wsClienteConectado) {
            return res.status(400).json({ error: 'El bot no estÃ¡ conectado a WhatsApp en este momento' });
        }
        ...
        const textoGuardar = caption && caption.trim() ? `ðŸ“· ${caption.trim()}` : 'ðŸ“· (InfografÃ­a / Imagen enviada)';
        const tsMs = Date.now();
...
// Obtener lista de respuestas rÃ¡pidas predeterminadas
...
// Guardar o editar respuesta rÃ¡pida
app.post('/api/respuestas-rapidas', autenticarToken, async (req, res) => {
    try {
        const { id, atajo, titulo, contenido } = req.body;
        if (!titulo || !contenido) return res.status(400).json({ error: 'TÃ­tulo y contenido son obligatorios' });
...
// Obtener enlaces rÃ¡pidos del sistema y formularios personalizables
...
// Guardar o editar enlace rÃ¡pido o formulario
app.post('/api/enlaces-rapidos', autenticarToken, async (req, res) => {
    try {
        const { id, titulo, descripcion, url, icono, color } = req.body;
        if (!titulo || !url) return res.status(400).json({ error: 'TÃ­tulo y URL son obligatorios' });
...
app.post('/api/citas/sincronizar-google', autenticarToken, async (req, res) => {
    try {
        ...
        const citasFaltantes = await allQuery("SELECT * FROM citas_agenda WHERE estado != 'Cancelada' AND (google_event_id IS NULL OR google_event_id = '') AND fecha >= ?", [hoyIso]);
        let subidasCont = 0;
        const timezone = (await getQuery("SELECT valor FROM configuracion WHERE clave = 'timezone'"))?.valor || 'America/Mexico_City';
        for (const cita of citasFaltantes) {
            try {
                const servicioNorm = (cita.servicio || '').toLowerCase();
                let durMin = (servicioNorm.includes('vasectom') || servicioNorm.includes('embarazo')) ? 60 : 30;
                const resGoogle = await calendarService.crearCita({
                    ...
                    notas: cita.notas || 'Sincronizado desde el panel',
                    timezone
                });
                if (resGoogle.success) {
                    await runQuery(
                        "UPDATE citas_agenda SET google_event_id = ?, google_calendar_id = ?, hora_fin = ?, link_evento = ? WHERE id = ?",
                        [resGoogle.eventId, calIdConfig, resGoogle.horaFin, resGoogle.htmlLink || '', cita.id]
                    );
                    subidasCont++;
                }
            } catch (err) {}
        ...
        if (citasActivas.length === 0) return res.json({ success: true, canceladas: 0, subidas: subidasCont });
        ...
        res.json({ success: true, canceladas: canceladasCont, subidas: subidasCont });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
});
...
app.post('/api/citas', autenticarToken, async (req, res) => {
    try {
        ...
        // Verificar si el mÃ³dulo de Google Calendar estÃ¡ encendido
        ...
        // Logica personalizada de duraciÃ³n
    ...
    try {
        ...
        if (moduloActivo && calIdConfig && credsConfig) {
            const resGoogle = await calendarService.crearCita({
                ...
                nombre: cliente_nombre || citaVieja.cliente_nombre,
                telefono: cliente_telefono || citaVieja.cliente_telefono,
...
app.delete('/api/citas/:id', autenticarToken, async (req, res) => {
    try {
        ...
        io.emit('cita_actualizada');
        res.json({ success: true, message: "Cita cancelada con Ã©xito" });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
});

// DiagnÃ³stico y prueba en vivo de conexiÃ³n con Google Calendar
...
app.get('/api/agenda/disponibilidad', autenticarToken, async (req, res) => {
    try {
        ...
        if (!fecha) return res.status(400).json({ error: "El parÃ¡metro fecha (YYYY-MM-DD) es requerido" });
...
app.post('/api/configuracion', autenticarToken, async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'ConfiguraciÃ³n actualizada en vivo' });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
});
...
// GestiÃ³n de Solicitudes de Asesor / Clientes en Espera
...
// GestiÃ³n de Documentos de Conocimiento (PDFs, Excel/CSV, TXT)
...
// GestiÃ³n de GalerÃ­a de ImÃ¡genes e InfografÃ­as (.png, .jpg, .webp)
...
// Auto-descubrimiento dinÃ¡mico de modelos oficiales de Google Gemini
let cacheModelosValidos = [];
let ultimoFetchModelos = 0;

// Sistema de Respaldo Local Inteligente ante caÃ­das o saturaciÃ³n (503) de Google AI
function generarRespuestaEmergencia(textoUsuario, config, estadoHorario) {
    const txt = (textoUsuario || '').toLowerCase();
    const icono = config.icono_asistente || 'ðŸ¤–';
    const negocio = config.nombre_negocio || 'nuestro establecimiento';

    if (txt.includes('donde') || txt.includes('dÃ³nde') || txt.includes('ubicacion') || txt.includes('ubicaciÃ³n') || txt.includes('direccion') || txt.includes('direcciÃ³n') || txt.includes('llegar')) {
        let resp = `${icono} ðŸ“ *UBICACIÃ“N DE ${negocio.toUpperCase()}*\n\n${config.ubicacion_direccion || 'Consulta con nuestro personal para indicaciones exactas.'}`;
        if (config.ubicacion_maps_link) resp += `\n\nðŸ—ºï¸ *Ver en Google Maps:*\n${config.ubicacion_maps_link}`;
        return resp;
    }

    if (txt.includes('horario') || txt.includes('hora') || txt.includes('abren') || txt.includes('cierran') || txt.includes('atienden') || txt.includes('dias') || txt.includes('dÃ­as')) {
        return `${icono} â° *HORARIOS DE ATENCIÃ“N*\n\n${config.horario_sucursal_fisica || 'Lunes a Viernes en horario de atenciÃ³n habitual.'}`;
    }

    if (txt.includes('costo') || txt.includes('precio') || txt.includes('cobran') || txt.includes('gratis') || txt.includes('pagar')) {
        let resp = `${icono} ðŸ’° *INFORMACIÃ“N DE COSTOS / SERVICIOS*\n\n`;
        if (config.catalogo_servicios) resp += `${config.catalogo_servicios}\n\n`;
        if (config.datos_bancarios) resp += `ðŸ’³ *MÃ©todos de pago:* ${config.datos_bancarios}`;
        return resp.trim();
    }

    if (txt.includes('requisito') || txt.includes('papel') || txt.includes('documento') || txt.includes('ine') || txt.includes('curp')) {
        return `${icono} ðŸ“‹ *REQUISITOS GENERALES*\n\nPara tu atenciÃ³n gratuita, presenta:\nâ€¢ Copia de INE o identificaciÃ³n oficial con fotografÃ­a\nâ€¢ Copia de CURP\n\n_Para mayores informes acude en nuestro horario de atenciÃ³n o escribe *5* para solicitar un asesor._`;
    }

    return `${icono} ðŸ¥ *Â¡Hola!* Por el momento mi sistema inteligente presenta una intermitencia temporal de conexiÃ³n.\n\n` +
        `Para ayudarte de inmediato:\n` +
        `â€¢ EnvÃ­a *MenÃº* para explorar todas nuestras opciones disponibles.\n` +
        `â€¢ Escribe *asesor* o elige la opciÃ³n 5 para solicitar atenciÃ³n personalizada.\n\n` +
        `_Agradecemos tu comprensiÃ³n._ âœ¨`;
}
...
async function obtenerModelosDisponibles(apiKey) {
    ...
    try {
        ...
        if (data && data.models && Array.isArray(data.models)) {
            ...
            // Priorizar explÃ­citamente gemini-3.6-flash y gemini-3-flash-preview (alta disponibilidad verificada)
        ...
        console.warn("âš ï¸ No se pudo consultar la lista dinÃ¡mica de modelos de Google:", e.message);
    }
...
// ============================================================================
// ENDPOINTS DEL SUPERADMIN (AuditorÃ­a)
// ============================================================================
...
function parsearComandoReceso(textoCompleto, tipoPorDefecto = 'curso') {
    ...
    const getMotivoDefault = (t) => {
        if (t === 'curso') return 'CapacitaciÃ³n y ActualizaciÃ³n Continua';
        if (t === 'festivo') return 'DÃ­a Festivo Oficial / InhÃ¡bil';
    ...
    if (!resto) {
        return {
            ...
            fechaFin: tipoPorDefecto === 'festivo' ? 'maÃ±ana a primera hora' : 'en breve'
        };
    }
    ...
    const ahora = new Date();
    const matchDias = restoLower.match(/(\d+)\s*(dias|dÃ­as)/);
    ...
    if (matchDias) {
        ...
        if (motivoLimpio && motivoLimpio.length > 2) motivo = motivoLimpio;
    } else if (restoLower.includes('maÃ±ana') || restoLower.includes('manana')) {
        const fechaRegreso = new Date(ahora.getTime() + 24 * 60 * 60 * 1000);
        const formatFecha = new Intl.DateTimeFormat('es-MX', { timeZone: 'America/Mexico_City', weekday: 'long', day: 'numeric', month: 'long' });
        fechaFin = `maÃ±ana (${formatFecha.format(fechaRegreso)})`;
        const motivoLimpio = resto.replace(/\b(hasta\s+)?(el\s+)?maÃ±ana\b/gi, '').trim();
        if (motivoLimpio && motivoLimpio.length > 2) motivo = motivoLimpio;
    } else {
        ...
        for (const [dia, diaNum] of Object.entries(diasSemana)) {
            ...
            if (regexDia.test(restoLower)) {
                encontradoDia = true;
                fechaFin = `el prÃ³ximo ${dia} a primera hora`;
    ...
    return {
        ...
        fechaFin: fechaFin || 'prÃ³ximamente'
    };
}

// Motor de CÃ¡lculo Inteligente de Horario en MÃ©xico (America/Mexico_City)
async function obtenerEstadoHorarioMexico() {
    ...
    if (ausenciaActivaManual) {
        ...
        // Consultar si hay algÃºn evento programado en el calendario para el dÃ­a de hoy (hora de MÃ©xico)
                        ...
                        if (calId && creds) {
                            ...
                            if (calEv && calEv.activo) {
                                eventoActivo = {
                                    ...
                                    motivo: calEv.titulo,
                                    fechaFin: calEv.fechaFin || 'prÃ³ximamente',
                                    esProgramado: true,
    ...
    if (eventoActivo) {
        ...
        let proximoTexto = esCurso
            ? 'al reanudar actividades tras la jornada de capacitaciÃ³n'
            : (esFestivo ? 'al reanudar labores tras el dÃ­a festivo oficial' : 'al reanudar actividades tras el periodo vacacional');
        ...
        if (eventoActivo.fechaFin) {
            ...
            if (/^\d{4}-\d{2}-\d{2}$/.test(fFinLower)) {
                ...
                proximoTexto = `el dÃ­a ${partes[2]}/${partes[1]}/${partes[0]} a las ${formatoHoraInicio}`;
            } else if (fFinLower.startsWith('maÃ±ana') || fFinLower.startsWith('hoy') || fFinLower.startsWith('el ') || fFinLower.startsWith('en ') || fFinLower.startsWith('al ')) {
                proximoTexto = `${eventoActivo.fechaFin.replace('a primera hora', '')} a las ${formatoHoraInicio}`.replace('  ', ' ');
            } else {
                proximoTexto = `el prÃ³ximo ${eventoActivo.fechaFin.replace('a primera hora', '')} a las ${formatoHoraInicio}`.replace('  ', ' ');
            }
        }
        return {
            enHorario: false,
            ...
            esFestivo,
            tipoReceso,
            motivoReceso: eventoActivo.motivo || (esCurso ? 'CapacitaciÃ³n y ActualizaciÃ³n Continua' : (esFestivo ? 'DÃ­a Festivo Oficial / InhÃ¡bil' : 'Periodo Vacacional')),
            proximoTexto,
            esProgramado: !!eventoActivo.esProgramado
        };
    }

    // Obtener configuraciÃ³n de textos para detectar fines de semana
    ...
    // Para saber si "los asesores" trabajan fin de semana, evaluamos estrictamente el horario en lÃ­nea (si estÃ¡ activo el switch)
    const textoBaseRevisar = difiereOnline ? horarioOnline.toLowerCase() : horarioFisico.toLowerCase();
    
    // HeurÃ­stica simple para saber si abren fines de semana basÃ¡ndose en el texto descriptivo
    let abreSabado = textoBaseRevisar.includes('sabado') || textoBaseRevisar.includes('sÃ¡bado') || textoBaseRevisar.includes('lunes a sabado') || textoBaseRevisar.includes('lunes a sÃ¡bado') || textoBaseRevisar.includes('lunes a domingo') || textoBaseRevisar.includes('todos los dias') || textoBaseRevisar.includes('todos los dÃ­as');
    let abreDomingo = textoBaseRevisar.includes('domingo') || textoBaseRevisar.includes('lunes a domingo') || textoBaseRevisar.includes('todos los dias') || textoBaseRevisar.includes('todos los dÃ­as');
    ...
    if (esCerrado('s[aÃ¡]bado|s[aÃ¡]b')) abreSabado = false;
    if (esCerrado('domingo|dom')) abreDomingo = false;

    // DÃ­as laborables dinÃ¡micos
    let diasLaborables = ['lun', 'mar', 'miÃ©', 'jue', 'vie'];
    ...
    if (esCerrado('mi[eÃ©]rcoles|mi[eÃ©]')) diasLaborables = diasLaborables.filter(d => d !== 'miÃ©');
    ...
    if (abreSabado) diasLaborables.push('sÃ¡b', 'sab');
    ...
    // HeurÃ­stica avanzada: si es fin de semana, intentar extraer el horario especÃ­fico del texto
    if (esDiaLaboral && (diaSemana.startsWith('s') || diaSemana.startsWith('d'))) {
        const regexDia = diaSemana.startsWith('s') 
            ? /(?:s[aÃ¡]bado|s[aÃ¡]b)[^\d]*(\d{1,2}(?::\d{2})?\s*(?:am|pm|a\.m\.|p\.m\.)?)\s*(?:a|al|hasta|-)\s*(\d{1,2}(?::\d{2})?\s*(?:am|pm|a\.m\.|p\.m\.)?)/i 
    ...
    if (esDiaLaboral && minutosActuales >= minInicioEfectivo && minutosActuales <= minFinEfectivo) {
        return {
            enHorario: true,
            enReceso: false,
            proximoTexto: 'actualmente en horario de atenciÃ³n'
        };
    }

    // Fuera de horario: calcular retorno amigable
    let proximoTexto = `en nuestro prÃ³ximo horario de atenciÃ³n (${formatoHoraInicio})`;
    
    if (!esDiaLaboral) {
        if (diaSemana.startsWith('s') && abreDomingo) {
            proximoTexto = `maÃ±ana domingo a partir de las ${formatoHoraInicio}`;
        } else if ((diaSemana.startsWith('s') || diaSemana.startsWith('d')) && (!abreSabado && !abreDomingo)) {
            proximoTexto = `el prÃ³ximo lunes a partir de las ${formatoHoraInicio}`;
        } else if (diaSemana.startsWith('d') && !abreDomingo) {
            proximoTexto = `maÃ±ana lunes a partir de las ${formatoHoraInicio}`;
        } else {
            proximoTexto = `maÃ±ana a partir de las ${formatoHoraInicio}`;
        }
...
app.post('/api/bot/festivo', autenticarToken, async (req, res) => {
    try {
        ...
        const motivoFinal = motivo || 'DÃ­a Festivo Oficial / InhÃ¡bil';
        await runQuery("INSERT INTO configuracion (clave, valor) VALUES ('ausencia_activa', ?) ON CONFLICT(clave) DO UPDATE SET valor = excluded.valor", [activa ? '1' : '0']);
...
app.post('/api/bot/eventos-ausencia', autenticarToken, async (req, res) => {
    try {
        ...
        res.json({ success: true, id: resultado.id, google_event_id: googleEventId, mensaje: "Evento programado con Ã©xito" });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
});
...
app.delete('/api/bot/eventos-ausencia/:id', autenticarToken, async (req, res) => {
    try {
        ...
        io.emit('eventos_ausencia_actualizados');
        res.json({ success: true, mensaje: "Evento eliminado con Ã©xito" });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
});
...
app.post('/api/bot/curso', autenticarToken, async (req, res) => {
    try {
        const { activa, motivo, fecha_fin } = req.body;
        const motivoFinal = motivo || 'CapacitaciÃ³n y ActualizaciÃ³n Continua';
        await runQuery("INSERT INTO configuracion (clave, valor) VALUES ('ausencia_activa', ?) ON CONFLICT(clave) DO UPDATE SET valor = excluded.valor", [activa ? '1' : '0']);
...
// Contador global de envÃ­os en vuelo (pendientes de resoluciÃ³n)
// Cuando message_create detecta que hay envÃ­os pendientes, sabe que ES un mensaje del bot.
...
client.sendMessage = async function(chatId, content, options) {
    ...
    botEnviosPendientes++;   // â† marca: hay un envÃ­o en vuelo
    try {
        ...
        ultimoEnvioBotMs = Date.now();                         // â† timestamp del Ãºltimo envÃ­o completado
...
client.on('loading_screen', (percent, message) => {
    ...
    // Si el bot YA estÃ¡ conectado y en ejecuciÃ³n, las sincronizaciones de fondo (50%, 99%) son normales y no deben recargar la pÃ¡gina
    ...
    // GuardiÃ¡n Universal: Si se queda en CUALQUIER porcentaje antes de 'ready' por mÃ¡s de 75 segundos sin avanzar, forzar recarga limpia
    ...
    if (tiempoInicioLoadingSaaS && (Date.now() - tiempoInicioLoadingSaaS > 75000)) {
        console.warn(`âš ï¸ ALERTA WATCHDOG: WhatsApp Web atascado en ${percent}% durante el inicio por mÃ¡s de 75s. Reiniciando proceso limpio con PM2...`);
        tiempoInicioLoadingSaaS = null;
        ultimoPorcentajeSaaS = null;
        process.exit(1);
    }
...
client.on('qr', (qr) => {
    ...
    console.log('ðŸ“² Escanea el cÃ³digo QR en tu aplicaciÃ³n de WhatsApp o en el Dashboard.');
});
...
client.on('ready', () => {
    ...
    console.log('ðŸš€ Â¡Motor OmniBot conectado y listo para atender clientes!');
});
...
client.on('disconnected', (reason) => {
    ...
    // Auto-reparaciÃ³n vÃ­a Auditor al detectar desconexiÃ³n fuerte
    if (typeof Auditor !== 'undefined') {
        Auditor.registrarEvento('CRITICO', `DesconexiÃ³n de WhatsApp detectada. RazÃ³n: ${reason}. Forzando reinicio para sanar...`).then(async () => {
...
// GuardiÃ¡n Activo y Keep-Alive periÃ³dico cada 60 segundos
setInterval(async () => {
    try {
        ...
        if (!wsClienteConectado && tiempoInicioLoadingSaaS && (Date.now() - tiempoInicioLoadingSaaS > 90000)) {
            console.warn(`âš ï¸ ALERTA WATCHDOG: El proceso de carga inicial lleva mÃ¡s de 90s atascado en ${ultimoPorcentajeSaaS}%. Reiniciando proceso limpio con PM2...`);
            tiempoInicioLoadingSaaS = null;
            ultimoPorcentajeSaaS = null;
            process.exit(1);
        }
...
// Helper seguro para simular "escribiendo..." sin que falle si el contexto de Puppeteer estÃ¡ ocupado
...
async function procesarMensajeEntrante(msg) {
    ...
    try {
        ...
        // DeduplicaciÃ³n estricta por ID de mensaje de WhatsApp
        ...
        // 1. Descartar mensajes antiguos (mÃ¡s de 2 minutos) que WhatsApp entrega al reconectar o reiniciar
        if (msg.timestamp) {
        ...
        if (texto && (texto.toLowerCase().includes('!reset') || texto.toLowerCase().includes('/reset') || texto.toLowerCase().includes('!borrar'))) {
            try {
                ...
                await client.sendMessage(remitente, 'âœ… *Memoria de Inteligencia Artificial borrada exitosamente.* El historial de este chat ha sido eliminado. Ya puedes enviar tu mensaje para iniciar de cero.');
            } catch(e) {}
            return;
        }
...
// FunciÃ³n para limpiar nombres y evitar saludos con cÃ³digos alfanumÃ©ricos, fechas, emojis o IDs tÃ©cnicos
function limpiarNombreParaSaludo(nombre) {
    if (!nombre) return '';
    const n = nombre.trim();
    // Extraer Ãºnicamente letras humanas (elimina emojis como ðŸ˜ˆðŸ–¤ðŸ˜‹ðŸº, sÃ­mbolos y nÃºmeros)
    const soloLetras = n.replace(/[^a-zA-ZÃ¡Ã©Ã­Ã³ÃºÃÃ‰ÃÃ“ÃšÃ±Ã‘\s]/g, '').trim();
    if (
        soloLetras.length < 2 ||
    ...
    // Si el contacto ya existÃ­a con un nombre editado o nÃºmero limpio, conservarlo
    ...
    // --------------------------------------------------------------------------
    // COMANDOS DE CONTROL MÃ“VIL Y GRUPO [CONTROL-BOT] (vCards y Comandos !)
    // --------------------------------------------------------------------------
    ...
    // Comprobar si el remitente es un telÃ©fono Administrador registrado (o tiene su LID vinculado)
    ...
    if (textoLower.startsWith('!soyadmin ')) {
        ...
        if (esValido) {
            ...
            await client.sendMessage(remitente, "âœ… Tu dispositivo (LID) ha sido vinculado exitosamente a tu nÃºmero de Administrador. Ya recibirÃ¡s alertas.");
            esAdminRemitente = true;
        } else {
            await client.sendMessage(remitente, "âŒ El nÃºmero que ingresaste no coincide con los configurados en el panel.");
        }
        return;
    }
    ...
    if (textoLower.startsWith('!admin ') || textoLower.startsWith('!clave ') || textoLower.startsWith('!vincularadmin ')) {
        ...
        if (!passIngresada) {
            const sent = await client.sendMessage(remitente, "âš ï¸ *Uso correcto:* EnvÃ­a `!admin TU_CONTRASEÃ‘A` (la contraseÃ±a que utilizas para entrar al panel web).");
            if (sent?.id) idsMensajesEnviadosBot.add(sent.id._serialized);
            return;
        }
        ...
        if (passCorrecta) {
            ...
            const sent = await client.sendMessage(remitente, 
                `âœ… *Â¡ADMINISTRADOR VINCULADO CON Ã‰XITO!*\n\n` +
                ...
                `â€¢ *!menu* -> Probar el menÃº de bienvenida\n` +
                `â€¢ *!curso [dÃ­as]* -> Activar modo capacitaciÃ³n\n` +
                `â€¢ *!auditoria* -> Reporte de servidor y RAM`
            );
            if (sent?.id) idsMensajesEnviadosBot.add(sent.id._serialized);
            return;
        } else {
            const sent = await client.sendMessage(remitente, "âŒ *ContraseÃ±a incorrecta.* Verifica la clave de acceso de tu panel web y vuelve a intentarlo con `!admin TU_CONTRASEÃ‘A`.");
            if (sent?.id) idsMensajesEnviadosBot.add(sent.id._serialized);
            return;
        }
    }
    ...
    if (textoLower === '!debugyo') {
        ...
        await client.sendMessage(remitente, 
            ...
            `Texto: [${textoLower}]\n\n` +
            `ðŸ’¡ _Para vincularte como admin envÃ­a:_ \`!admin TU_CONTRASEÃ‘A\``
        );
        return;
    }
    ...
    if (esGrupo || (textoLower.startsWith('!') && esAdminRemitente) || (esVCard && esAdminRemitente)) {
        ...
        if (esVCard) {
            ...
            if (numExtraido) {
                ...
                const sent = await msg.reply(`ðŸš« *Contacto Ignorado con Ã©xito:*\nðŸ“± NÃºmero: +${numExtraido}\n\nEl bot ya no le responderÃ¡ a esta persona.`);
                if (sent?.id) idsMensajesEnviadosBot.add(sent.id._serialized);
                return;
            }
        }

        // 2. Comandos con signo de exclamaciÃ³n (!)
        if (textoLower.startsWith('!')) {
            ...
            if (textoLower === '!probar' || textoLower === '!prueba' || textoLower === '!probar on' || textoLower === '!prueba on' || textoLower === '!modo prueba on' || textoLower === '!modo prueba') {
                ...
                const sent = await client.sendMessage(remitente, "ðŸ§ª *MODO PRUEBA ACTIVADO.*\n\nAhora el bot te responderÃ¡ en este chat exactamente como si fueras un cliente o cliente nuevo.\n\n_Para desactivarlo envÃ­a `!probar off` o `!reactivar`._");
                if (sent?.id) idsMensajesEnviadosBot.add(sent.id._serialized);
                return;
            }
            ...
            // ------------------------------------------------------------------
            // COMANDO: !curso / !congreso / !capacitacion / !taller (Modo ActualizaciÃ³n MÃ©dica)
            // ------------------------------------------------------------------
            if (textoLower.startsWith('!curso') || textoLower.startsWith('!congreso') || textoLower.startsWith('!capacitacion') || textoLower.startsWith('!capacitaciÃ³n') || textoLower.startsWith('!taller')) {
                const parsed = parsearComandoReceso(texto, 'curso');
                if (!parsed.activo) {
                    await runQuery("INSERT INTO configuracion (clave, valor) VALUES ('ausencia_activa', '0') ON CONFLICT(clave) DO UPDATE SET valor = '0'");
                    io.emit('estado_control_actualizado', { ausenciaActiva: false, ausenciaTipo: 'curso' });
                    const sent = await client.sendMessage(remitente, "ðŸŽ“ *MODO CURSO / CONGRESO DESACTIVADO.*\n\nEl bot y el equipo de salud reanudan la atenciÃ³n y agenda de citas presenciales habitual.");
                    if (sent?.id) idsMensajesEnviadosBot.add(sent.id._serialized);
                    return;
                }
                ...
                const sent = await client.sendMessage(remitente,
                    `ðŸŽ“ *MODO CURSO / CONGRESO MÃ‰DICO ACTIVADO*\n` +
                    `â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n` +
                    `ðŸ“Œ *Motivo:* ${parsed.motivo}\n` +
                    `ðŸ—“ï¸ *ReanudaciÃ³n estimada:* ${parsed.fechaFin}\n` +
                    `ðŸ¤– *Rol de la IA:* Activa 24/7 explicando con calidez que el equipo estÃ¡ en ActualizaciÃ³n Continua, resolviendo dudas sobre mÃ©todos y apartando citas con prioridad para el regreso.\n` +
                    `â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n` +
                    `ðŸ’¡ _Para desactivar envÃ­a \`!curso off\` o \`!reactivar\`._`
                );
                if (sent?.id) idsMensajesEnviadosBot.add(sent.id._serialized);
                return;
            }
            ...
            if (textoLower.startsWith('!vacaciones') || textoLower.startsWith('!ausencia')) {
                ...
                if (!parsed.activo) {
                    ...
                    const sent = await client.sendMessage(remitente, "ðŸ–ï¸ *MODO VACACIONES DESACTIVADO.* El bot reanuda la atenciÃ³n normal.");
                    if (sent?.id) idsMensajesEnviadosBot.add(sent.id._serialized);
                    return;
                }
                ...
                const sent = await client.sendMessage(remitente,
                    ...
                    `â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n` +
                    `ðŸ“Œ *Motivo:* ${parsed.motivo}\n` +
                    `ðŸ—“ï¸ *ReanudaciÃ³n:* ${parsed.fechaFin}\n` +
                    `â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n` +
                    `ðŸ’¡ _Para desactivar envÃ­a \`!reactivar\` o \`!vacaciones off\`._`
                );
                if (sent?.id) idsMensajesEnviadosBot.add(sent.id._serialized);
                return;
            }
            ...
            // ------------------------------------------------------------------
            // COMANDO: !festivo / !feriado / !asueto / !inhabil (Modo SuspensiÃ³n Oficial de Labores)
            // ------------------------------------------------------------------
            if (textoLower.startsWith('!festivo') || textoLower.startsWith('!feriado') || textoLower.startsWith('!asueto') || textoLower.startsWith('!inhabil') || textoLower.startsWith('!inhÃ¡bil')) {
                ...
                if (!parsed.activo) {
                    ...
                    const sent = await client.sendMessage(remitente, "ðŸ‡²ðŸ‡½ *MODO DÃA FESTIVO DESACTIVADO.*\n\nEl bot y el personal reanudan la atenciÃ³n y agenda de citas presenciales habitual.");
                    if (sent?.id) idsMensajesEnviadosBot.add(sent.id._serialized);
                    return;
                }
                ...
                const sent = await client.sendMessage(remitente,
                    ...
                    `â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n` +
                    `ðŸ“Œ *ConmemoraciÃ³n / Motivo:* ${parsed.motivo}\n` +
                    `ðŸ—“ï¸ *ReanudaciÃ³n estimada:* ${parsed.fechaFin}\n` +
                    `ðŸ¤– *Rol de la IA:* Activa 24/7 resolviendo dudas sobre el catÃ¡logo y servicios del negocio, anotando solicitudes en lista prioritaria para el regreso.\n` +
                    `â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n` +
                    `ðŸ’¡ _Para desactivar envÃ­a \`!festivo off\` o \`!reactivar\`._`
                );
                if (sent?.id) idsMensajesEnviadosBot.add(sent.id._serialized);
                return;
            }
            ...
            if (textoLower.startsWith('!atender')) {
                ...
                if (num) {
                    ...
                    const sent = await client.sendMessage(remitente, `âœ… *Contacto +${num} removido de ignorados.* El bot volverÃ¡ a atenderlo.`);
                    if (sent?.id) idsMensajesEnviadosBot.add(sent.id._serialized);
                }
                return;
            }
            ...
            if (textoLower === '!pendientes' || textoLower === '!resumen') {
                ...
                if (ultimos.length === 0) {
                    rep += `_AÃºn no hay clientes recientes registrados (los registros de prueba se han limpiado)._\n`;
                } else {
                    ultimos.forEach((u, i) => {
            ...
            if (textoLower === '!ayuda' || textoLower === '!help') {
                const sent = await client.sendMessage(remitente, 
                    "ðŸ¤– *COMANDOS DISPONIBLES DE CONTROL OMNIBOT:*\n\n" +
                    ...
                    "â¸ï¸ `!pausa 4111234567` -> Pausa a un cliente especÃ­fico.\n" +
                    ...
                    "ðŸ“‹ `!menu` -> Muestra el menÃº numÃ©rico interactivo.\n" +
                    "ðŸš« `!ignorar 4111234567` -> Agrega a la lista de ignorados.\n" +
                    "âœ… `!atender 4111234567` -> Remueve de ignorados.\n" +
                    "ðŸ“‹ `!resumen` -> Lista los Ãºltimos clientes atendidos.\n" +
                    "ðŸ›¡ï¸ `!auditoria` -> DiagnÃ³stico del servidor, memoria RAM y salud del bot.\n" +
                    "ðŸ”‘ `!admin [contraseÃ±a]` -> Vincular tu WhatsApp como Administrador."
                );
                if (sent?.id) idsMensajesEnviadosBot.add(sent.id._serialized);
                return;
            }

            if (textoLower === '!menu' || textoLower === '!menÃº') {
                const nombreMostrar = (nombreContacto && nombreContacto !== 'Cliente') ? nombreContacto : null;
                const saludoHeader = nombreMostrar ?
                    `${iconoAsistente ? iconoAsistente + ' ' : ''}ðŸ‘‹ *Â¡Hola, ${nombreMostrar}! Bienvenido(a) a ${nombreNegocio}.*` :
                    `${iconoAsistente ? iconoAsistente + ' ' : ''}ðŸ‘‹ *Â¡Hola! Bienvenido(a) a ${nombreNegocio}.*`;

                const horarioFisico = (await getQuery("SELECT valor FROM configuracion WHERE clave = 'horario_sucursal_fisica'"))?.valor || '';
                let textoMenu = `${saludoHeader}\n\nÂ¡Estamos para servirte! ðŸ¤–\n\nElige una opciÃ³n:\n\n`;
                try {
    ...
    // --------------------------------------------------------------------------
    // VERIFICACIÃ“N DE PAUSA INDIVIDUAL POR INTERVENCIÃ“N HUMANA
    // --------------------------------------------------------------------------
    let estaPausado = chatsPausados.has(remitente);
    let tiempoPausa = chatsPausados.get(remitente) || 0;

    // Buscar si estÃ¡ pausado bajo su nÃºmero limpio (si remitente es @lid o viceversa)
    ...
    if (estaPausado) {
        ...
        if (transcurrido < minsPausa * 60 * 1000) {
            ...
            console.log(`â¸ï¸ Chat ${remitente} (Tel: ${telefonoReal}) en PAUSA por intervenciÃ³n humana (quedan ${minutosRestantes} mins). Silencio total.`);
            return;
        }
    ...
    // Bloqueo de concurrencia inteligente con expiraciÃ³n automÃ¡tica TTL (12s)
    if (!esGrupo) {
        const tiempoInicioProc = chatsEnProceso.get(remitente);
        if (tiempoInicioProc && (Date.now() - tiempoInicioProc < 12000)) {
            console.log(`â³ Chat ${remitente} ya estÃ¡ siendo procesado concurrentemente. Omitiendo respuesta duplicada.`);
            return;
        }
    ...
    const iconoAsistente = (await getQuery("SELECT valor FROM configuracion WHERE clave = 'icono_asistente'"))?.valor || 'ðŸ¤–';
    const nombreNegocio = (await getQuery("SELECT valor FROM configuracion WHERE clave = 'nombre_negocio'"))?.valor || 'nuestro negocio';
        ...
        if (notificarActiva && palabrasRaw.trim()) {
            ...
            if (palabraEncontrada) {
                ...
                if (Date.now() - ultimoEnvio > 15 * 60 * 1000) {
                    ...
                    const alertaMsg = `ðŸš¨ *ALERTA OMNIBOT - PALABRA CLAVE DETECTADA* ðŸš¨\n\n` +
                        ...
                        `ðŸ‘‰ _Puedes responderle directamente abriendo su conversaciÃ³n en WhatsApp o en el Panel._`;

                    // 1. Enviar a NÃºmeros Administradores y LIDs vinculados
    ...
    if (chatsEsperandoNombre.has(remitente) && !texto.startsWith('!')) {
        ...
        const frasesNoNombre = [
            'ya lo hice', 'ya lo llene', 'ya lo llenÃ©', 'ya quedo', 'ya quedÃ³', 'listo', 'ya estÃ¡', 'ya esta',
            'ya envie', 'ya enviÃ©', 'ya mande', 'ya mandÃ©', 'ok', 'si', 'ya', 'gracias', 'hola', 'buenas',
            'hecho', 'completado', 'registrado', 'formulario', 'link', 'enlace', 'ya registre', 'ya registrÃ©',
            'ya puse', 'listo ya', 'ya fue', 'ya terminÃ©', 'ya termine'
        ];

        const esConfirmacionSinNombre = frasesNoNombre.some(f => txtLower === f || txtLower.startsWith(f + ' ') || txtLower.endsWith(' ' + f));
        const esPreguntaOServicio = /\b(qu[eÃ©]|cu[aÃ¡]nto|cu[aÃ¡]ndo|c[oÃ³]mo|d[oÃ³]nde|por qu[eÃ©]|tienen|tienes|costo|precio|servicio|horario|ubicaci[oÃ³]n|requisito|cat[aÃ¡]logo|producto|disponible|pedido|entrega|env[iÃ­]o|cita|talla|oferta|descuento)\b/i.test(txtLower);

        if (esPreguntaOServicio) {
            // Si el cliente envÃ­a una duda o duda o pregunta, liberar la espera y responderle su duda
            ...
            const msjPedirNombre = `${iconoAsistente ? iconoAsistente + ' ' : ''}Â¡Excelente! âœï¸ Para registrarte e identificarte con nuestro equipo, por favor indÃ­came **cuÃ¡l es tu nombre** (o cÃ³mo te gustarÃ­a que te llamemos):`;
        ...
        if (estadoHorario.enReceso) {
            if (estadoHorario.esFestivo) {
                msjConfirmado = `${iconoAsistente ? iconoAsistente + ' ' : ''}Â¡Muchas gracias, *${nombreLimpio}*! Tu registro y aviso de privacidad han sido confirmados con Ã©xito. âœï¸âœ…\n\n` +
                    `ðŸ‡²ðŸ‡½ Con motivo del dÃ­a festivo oficial (*${estadoHorario.motivoReceso}*), has quedado registrado(a) con prioridad en nuestra **Lista de Espera Prioritaria** y te contactaremos **${estadoHorario.proximoTexto}**.\n\n` +
                    `ðŸ’¬ *Â¡El asistente virtual sigue activo para ti!* Puedes preguntarme sobre nuestros productos, catÃ¡logo, precios o disponibilidad y con gusto resolverÃ© tus dudas al instante. â˜ºï¸`;
            } else if (estadoHorario.esCurso) {
                msjConfirmado = `${iconoAsistente ? iconoAsistente + ' ' : ''}Â¡Muchas gracias, *${nombreLimpio}*! Tu registro y aviso de privacidad han sido confirmados con Ã©xito. âœï¸âœ…\n\n` +
                    `ðŸŽ“ nuestro equipo se encuentra en jornadas de capacitaciÃ³n continua (*${estadoHorario.motivoReceso}*). Has quedado registrado(a) con prioridad en nuestra **Lista de Espera Prioritaria** y te contactaremos **${estadoHorario.proximoTexto}**.\n\n` +
                    `ðŸ’¬ *Â¡El asistente virtual sigue activo para ti!* Puedes preguntarme sobre nuestros productos, catÃ¡logo, precios o disponibilidad y con gusto resolverÃ© tus dudas al instante. â˜ºï¸`;
            } else {
                msjConfirmado = `${iconoAsistente ? iconoAsistente + ' ' : ''}Â¡Muchas gracias, *${nombreLimpio}*! Tu registro y aviso de privacidad han sido confirmados con Ã©xito. âœï¸âœ…\n\n` +
            ...
            msjConfirmado = `${iconoAsistente ? iconoAsistente + ' ' : ''}Â¡Muchas gracias, *${nombreLimpio}*! Tu registro y aviso de privacidad han sido confirmados con Ã©xito. âœï¸âœ…\n\n` +
                `â° *Fuera de horario de atenciÃ³n en lÃ­nea:* He dejado tu solicitud registrada. Nuestro personal te responderÃ¡ por este chat **${estadoHorario.proximoTexto}**.\n\n` +
            ...
            msjConfirmado = `${iconoAsistente ? iconoAsistente + ' ' : ''}Â¡Muchas gracias, *${nombreLimpio}*! Tu registro y aviso de privacidad han sido confirmados con Ã©xito. âœï¸âœ…\n\n` +
                `He notificado a nuestro equipo de ${nombreNegocio}. En un momento te atenderÃ¡n de forma personalizada.\n\n` +
                `_Mientras tanto, el asistente virtual se mantiene activo 24/7 por si deseas consultar nuestros servicios o requisitos._`;
        }
    ...
    // --------------------------------------------------------------------------
    // B. SALUDO INICIAL Y MENÃš INTERACTIVO DE BIENVENIDA (OPCIONAL)
    // --------------------------------------------------------------------------
    const saludos = [
        'hola', 'buenas', 'buenos dias', 'buen dia', 'buenos dÃ­as', 'buen dÃ­a',
        'buenas tardes', 'buenas noches', 'menu', 'menÃº', 'inicio', 'opciones',
        'empezar', 'hola!', 'hola buenas', 'hola buen dia', 'hola buenos dias',
        'hola buenas tardes', 'hola buenas noches', 'saludos', 'ola', 'holaa', 'holaaa',
        'que tal', 'quÃ© tal', 'saludos cordiales'
    ];
    
    // Inactividad: si pasaron mÃ¡s de 60 minutos (1 hora) sin interacciÃ³n, o si es primera vez, o si estÃ¡ en modo prueba
    ...
    const esNuevaConversacion = tiempoInactivo > (12 * 60 * 60 * 1000); // 12 horas (mÃ¡ximo 1 menÃº de bienvenida por jornada/dÃ­a)
    const pideMenuExplicito = ['menu', 'menÃº', 'inicio', 'opciones', 'empezar', '!menu', '!menÃº', 'ver menu', 'ver menÃº'].includes(textoLowerNorm);
    ...
    if ((pideMenuExplicito || esNuevaConversacion) && mostrarMenuNumerico) {
        ...
        await runQuery(
            "INSERT INTO mensajes (chat_id, emisor, emisor_nombre, cuerpo, es_mio, es_ia, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)",
            [remitente, 'bot', 'MenÃº Bienvenida', textoMenu, 1, 1, Date.now()]
        );
        return;
    }
    ...
    try {
        ...
        if (menuConfigRaw) {
            ...
            const opcionAsesorObj = menuOpts.find(o => /\b(asesor|humano|persona|agente|personal|transferir|agendar|cita|cotizaci[oÃ³]n|cotizaciones)\b/i.test(o.titulo + ' ' + (o.respuesta || '')));
            if (opcionAsesorObj && opcionAsesorObj.opcion.toString().trim() === texto.trim()) {
                esOpcionMenuAsesor = true;
                tituloOpcionAsesor = `MenÃº OpciÃ³n ${opcionAsesorObj.opcion}: ${opcionAsesorObj.titulo}`;
            }
        }
    } catch(e) {}

    const regexPideAsesor = /\b(asesor|humano|persona|agente|personal|transferir|agendar|cita|atenci[oÃ³]n presencial|cotizaci[oÃ³]n|cotizaciones)\b/i;
    
    // Pide asesor si es opciÃ³n de menÃº o si solicita hablar con alguien/agendar cita
    ...
    if (pideAsesorDirecto) {
        ...
        if (estadoHorario.enReceso) {
            if (estadoHorario.esFestivo) {
                msjTransferido = `${iconoAsistente ? iconoAsistente + ' ' : ''}ðŸ‡²ðŸ‡½ *Aviso de DÃ­a Festivo / InhÃ¡bil Oficial:*\n` +
                    `${saludoPersonal} Te informamos que hoy es dÃ­a festivo oficial con suspensiÃ³n de labores presenciales (*${estadoHorario.motivoReceso}*).\n\n` +
                    `ðŸ—“ï¸ Tu solicitud para atenciÃ³n personalizada ha quedado registrada en nuestra **Lista de Espera Prioritaria**. nuestro equipo se comunicarÃ¡ contigo **${estadoHorario.proximoTexto}**.\n\n` +
                    `ðŸ’¬ *Â¡El asistente virtual sigue 100% activo en este chat!* Puedo resolverte al instante cualquier duda sobre el catÃ¡logo, productos, precios o disponibilidad. Â¡Con gusto te ayudo de inmediato! â˜ºï¸`;
            } else if (estadoHorario.esCurso) {
                msjTransferido = `${iconoAsistente ? iconoAsistente + ' ' : ''}ðŸŽ“ *Aviso de CapacitaciÃ³n / ActualizaciÃ³n:*\n` +
                    `${saludoPersonal} En este momento nuestro equipo se encuentra en jornadas de capacitaciÃ³n continua (*${estadoHorario.motivoReceso}*) para brindarte el mejor servicio.\n\n` +
                    `ðŸ—“ï¸ Tu solicitud para atenciÃ³n personalizada ha quedado registrada en nuestra **Lista de Espera Prioritaria**. nuestro equipo se comunicarÃ¡ contigo **${estadoHorario.proximoTexto}**.\n\n` +
                    `ðŸ’¬ *Â¡El asistente virtual sigue 100% activo en este chat!* Puedo resolverte cualquier duda sobre el catÃ¡logo, productos, precios o disponibilidad. Â¡Con gusto te ayudo! â˜ºï¸`;
            } else {
                msjTransferido = `${iconoAsistente ? iconoAsistente + ' ' : ''}ðŸŒ´ *Aviso de Receso / Vacaciones:*\n` +
                    ...
                    `ðŸ—“ï¸ Tu solicitud para atenciÃ³n personalizada ha quedado registrada en espera. El equipo de atenciÃ³n se comunicarÃ¡ contigo **${estadoHorario.proximoTexto}**.\n\n` +
                    `ðŸ’¬ *Â¡El asistente virtual sigue 100% activo 24/7!* Con gusto puedo resolver cualquier duda sobre el catÃ¡logo, productos o precios.`;
            }
        } else if (!estadoHorario.enHorario) {
            msjTransferido = `${iconoAsistente ? iconoAsistente + ' ' : ''}â° *Fuera de Horario de AtenciÃ³n en LÃ­nea:*\n` +
                `${saludoPersonal} Nuestro horario de atenciÃ³n en lÃ­nea es: ${horarioAtencionFinal || 'en nuestro horario habitual'}.\n\n` +
                `ðŸ•’ Tu solicitud ha quedado registrada. Nuestro equipo te responderÃ¡ y atenderÃ¡ **${estadoHorario.proximoTexto}**.`;
        } else {
            msjTransferido = `${iconoAsistente ? iconoAsistente + ' ' : ''}ðŸ‘¨â€âš•ï¸ ${saludoEntendido} He notificado a nuestro equipo de ${nombreNegocio} por este chat.\n\n` +
                `ðŸ•’ Nuestro equipo en turno revisarÃ¡ tus mensajes y te responderÃ¡ por aquÃ­ en cuanto se desocupe.\n\n` +
        ...
        // Si el cliente aÃºn no tiene su nombre registrado o no ha llenado el formulario de privacidad:
        if (!nombreContacto || nombreContacto === 'Cliente' || nombreContacto.startsWith('Cliente (+')) {
            msjTransferido += `\n\nðŸ“‹ *Para agilizar tu turno al reanudar:* Si aÃºn no has llenado tu registro previo, por favor completa este enlace:\nðŸ‘‰ ${enlacePrivacidad}\n\nâœï¸ Y escrÃ­benos aquÃ­ tu *Nombre Completo* para apartar tu lugar en la lista.`;
    ...
    // --------------------------------------------------------------------------
    // D. EVALUACIÃ“N DE OPCIONES DEL MENÃš NUMÃ‰RICO (1, 2, 3...)
    // --------------------------------------------------------------------------
    try {
        const menuConfigRaw = (await getQuery("SELECT valor FROM configuracion WHERE clave = 'menu_numerico'"))?.valor;
        if (menuConfigRaw) {
            ...
            if (opcionEncontrada) {
                ...
                await runQuery(
                    "INSERT INTO mensajes (chat_id, emisor, emisor_nombre, cuerpo, es_mio, es_ia, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    [remitente, 'bot', 'MenÃº Interactivo', respMenu, 1, 1, Date.now()]
                );

                io.emit('nuevo_mensaje', {
                    chat_id: remitente,
                    emisor: 'bot',
                    emisor_nombre: 'MenÃº Interactivo',
                    cuerpo: respMenu,
                    es_mio: 1,
                    es_ia: 1,
                    timestamp: Date.now()
                ...
                // Si la opciÃ³n seleccionada es especÃ­ficamente para solicitar asesor o humano, registrar la solicitud
                ...
                if (esOpcionAsesor) {
                    try {
                        ...
                        if (!yaExiste) {
                            await runQuery(
                                "INSERT INTO solicitudes_asesor (jid, telefono, nombre, motivo, fecha_hora, timestamp, estado) VALUES (?, ?, ?, ?, ?, ?, 'pendiente')",
                                [remitente, telLimpio, (nombreContacto && nombreContacto !== 'Cliente') ? nombreContacto : 'Cliente', `MenÃº OpciÃ³n ${opcionEncontrada.opcion}: ${opcionEncontrada.titulo}`, obtenerFechaHoraLocal(), Date.now()]
                            );
                            io.emit('solicitud_asesor_actualizada');
                        }
                    } catch(eSol) {}
    ...
    // --------------------------------------------------------------------------
    // 2. EVALUACIÃ“N Y ENVÃO AUTOMÃTICO DE INFOGRAFÃAS Y FOTOGRAFÃAS (.png, .jpg)
    // --------------------------------------------------------------------------
    ...
    async function enviarImagenSiExiste(palabraClave, captionTitulo) {
        ...
        const pideExplicito = textoLowerNorm.includes('ver') || textoLowerNorm.includes('imagen') || textoLowerNorm.includes('foto') || textoLowerNorm.includes('infografia') || textoLowerNorm.includes('infografÃ­a');
    ...
    // Auto-detecciÃ³n dinÃ¡mica de cualquier imagen en la carpeta segÃºn las palabras del cliente
    try {
        ...
        console.error("Error buscando imÃ¡genes automÃ¡ticas:", errGaleria);
    }

    // Las imÃ¡genes se detectan automÃ¡ticamente por nombre de archivo (sistema auto-detecciÃ³n arriba).
    // Para aÃ±adir imÃ¡genes por palabra clave, configÃºralas en el panel â†’ InfografÃ­as, o
    // sube archivos a /imagenes con nombres descriptivos (ej: "uniforme_quirurgico.jpg", "calzado_enfermeria.jpg").
    if (textoLowerNorm.includes('catalogo') || textoLowerNorm.includes('catÃ¡logo') || textoLowerNorm.includes('productos')) {
        await enviarImagenSiExiste('catalogo', 'ðŸ–¼ï¸ *CatÃ¡logo de Productos*');
        await enviarImagenSiExiste('catalogo_general', 'ðŸ–¼ï¸ *CatÃ¡logo General*');
    ...
    try {
        ...
        if (infografiasRaw) {
            ...
            if (infoEncontrada) {
                ...
                await runQuery(
                    "INSERT INTO mensajes (chat_id, emisor, emisor_nombre, cuerpo, es_mio, es_ia, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    [remitente, 'bot', 'InfografÃ­a IA', respInfo, 1, 1, Date.now()]
                );
                return;
            }
        }
    ...
    // --------------------------------------------------------------------------
    // 3. GENERACIÃ“N DE RESPUESTA CON GEMINI AI (MODELOS DINÃMICOS Y API KEY PROPIA)
    // --------------------------------------------------------------------------
    try {
        const customApiKey = (await getQuery("SELECT valor FROM configuracion WHERE clave = 'gemini_api_key'"))?.valor;
        const activeKey = (customApiKey && customApiKey.trim()) ? customApiKey.trim() : geminiApiKey;
        ...
        if (menuConfigRawIA) {
            try {
                ...
                menuOpts.forEach(o => {
                    if (o.titulo) textoOpcionesMenuIA += `- OpciÃ³n ${o.opcion}: ${o.titulo} | Respuesta: ${o.respuesta || ''} | Enlace exacto: ${o.enlace || 'Ninguno'}\n`;
                });
            } catch(e) {}
        }
        
        // Estado de ausencia gestionado en reglaHorarioIA (bloque Ãºnico, sin redundancias)

        // Historial reciente de la conversaciÃ³n (unificando por JID y telÃ©fono)
        ...
        let contextoHistorial = historialSinUltimo.reverse().map(m => {
            let txt = m.cuerpo || '';
            if (txt.startsWith('/9j/') || txt.startsWith('data:image')) txt = 'ðŸ“· (InfografÃ­a / Imagen enviada)';
...
async function obtenerContenidoGoogleSheets(url) {
    ...
    try {
        ...
        // Carga de Documentos Externos de Conocimiento (PDFs, TXT, CatÃ¡logos)
...
- REGLAS DE ATENCIÃ“N CON IA:
  1. Â¡HAZ TU TRABAJO NORMAL! Responde con calidez y detalle cualquier duda sobre los productos, servicios o catÃ¡logo del negocio, costos, disponibilidad y requisitos.
  ...
  3. SÃ“LO si el cliente pide turno presencial o asesor humano, aclara que la atenciÃ³n humana se reanuda ${estadoHorario.proximoTexto} por dÃ­a festivo oficial, y que queda anotado(a) en la Lista de Espera Prioritaria.
  4. PROHIBIDO ofrecer un asesor para hoy; la atenciÃ³n humana serÃ¡ hasta ${estadoHorario.proximoTexto}.`;
            } else if (estadoHorario.esCurso) {
                reglaHorarioIA = `
ðŸŽ“ ESTADO DE CAPACITACIÃ“N / CONGRESO MÃ‰DICO:
- El equipo de atenciÃ³n se encuentra en: "${estadoHorario.motivoReceso}".
- REGLAS DE ATENCIÃ“N CON IA:
  1. Â¡HAZ TU TRABAJO NORMAL! Responde de inmediato cualquier duda sobre el catÃ¡logo, productos, servicios, costos y disponibilidad.
  2. NO menciones que el equipo estÃ¡ en capacitaciÃ³n a menos que el cliente pida un turno presencial o hablar con el personal.
  3. SÃ“LO si solicita turno presencial o hablar con un asesor, explica que el equipo se encuentra en actualizaciÃ³n continua y que la atenciÃ³n humana se reanuda ${estadoHorario.proximoTexto}, dejÃ¡ndolo anotado en la Lista de Espera Prioritaria.
  4. PROHIBIDO ofrecer un asesor para hoy; la atenciÃ³n humana serÃ¡ hasta ${estadoHorario.proximoTexto}.`;
            } else {
                reglaHorarioIA = `
ðŸ”´ ESTADO DE RECESO / VACACIONES:
- Personal en receso debido a: "${estadoHorario.motivoReceso}".
- REGLA: Responde normalmente las dudas sobre el catÃ¡logo. SÃ“LO si pide cita o asesor, aclara que se reanudan ${estadoHorario.proximoTexto}.`;
            }
        } else if (!estadoHorario.enHorario) {
            reglaHorarioIA = `
ðŸ”´ ESTADO DE HORARIO DE ATENCIÃ“N (FUERA DE HORARIO DE ATENCIÃ“N POR CHAT):
- Fecha y hora actual en MÃ©xico: ${obtenerFechaHoraLocal()}.
- Actualmente estamos FUERA del horario en que el personal humano responde mensajes por este chat. El personal responderÃ¡ mensajes por WhatsApp: ${estadoHorario.proximoTexto}.
- REGLAS ESTRICTAS DE HORARIO Y CITAS (NO CONFUNDIR ATENCIÃ“N EN LÃNEA CON ATENCIÃ“N FÃSICA):
  1. NUNCA le digas al cliente que puede acudir o presentarse fÃ­sicamente sin haber coordinado previamente por este chat.
  2. Aclara que el horario de atenciÃ³n en lÃ­nea (${horarioAtencionFinal || 'el horario habitual de atenciÃ³n'}) es para responder dudas por WhatsApp y atenciÃ³n humana.
  3. Para cualquier atenciÃ³n presencial, el cliente debe tener una cita confirmada.
  4. AdviÃ©rtele amablemente que no visite las instalaciones sin haber coordinado previamente.
  5. SI CUENTAS CON EL MÃ“DULO DE AGENDAMIENTO AUTOMÃTICO (lee mÃ¡s abajo), puedes ofrecerle los horarios disponibles y agendar su cita de inmediato. Si NO cuentas con disponibilidad, confÃ­rmale que su solicitud quedÃ³ registrada para coordinarla en cuanto el personal inicie su turno.`;
        } else {
            reglaHorarioIA = `
ðŸŸ¢ ESTADO DE HORARIO DE ATENCIÃ“N (DENTRO DE HORARIO DE CHAT):
- Fecha y hora actual en MÃ©xico: ${obtenerFechaHoraLocal()}.
- Actualmente el equipo humano del negocio estÃ¡ EN TURNO atendiendo mensajes por este chat.
- REGLA DE CONTINUIDAD: Responde tÃº con calidez y precisiÃ³n cualquier duda del cliente sobre el catÃ¡logo, productos, servicios, requisitos y disponibilidad. NUNCA le digas que 'escriba asesor' o que 'hable con un asesor' si tÃº tienes la informaciÃ³n para resolver su duda o si la conversaciÃ³n ya estÃ¡ en curso.`;
        }
        ...
        const reglaHorarioBase = estadoHorario.enReceso
            ? `3. REGLA ESTRICTA POR ${estadoHorario.esFestivo ? 'DÃA FESTIVO OFICIAL' : (estadoHorario.esCurso ? 'capacitaciÃ³n' : 'RECESO')}: Actualmente ${estadoHorario.esFestivo ? 'es dÃ­a festivo oficial no laborable' : (estadoHorario.esCurso ? 'el equipo de atenciÃ³n se encuentra en jornadas de capacitaciÃ³n' : 'el personal se encuentra en receso vacacional')}. Las citas presenciales y la agenda se reanudan: ${estadoHorario.proximoTexto}. PROHIBIDO TERMINANTEMENTE decir que el personal atenderÃ¡ a las 2:00 PM de hoy mientras estemos en festivo/receso.`
            : `3. El horario configurado (${horarioAtencionFinal || 'el horario habitual de atenciÃ³n'}) es de ATENCIÃ“N EN LÃNEA POR WHATSAPP para resolver dudas y coordinar citas o pedidos.`;

        // MÃ³dulo Universal de Agendamiento Automatizado con Google Calendar
...
`;
            ...
            seccionAgendaIA = `
- REGLA DE DETECCIÃ“N DE CITAS (CRÃTICO): Si el usuario te confirma que desea agendar una cita, apartar un turno, o solicita hablar con el personal humano, DEBES incluir obligatoriamente la etiqueta oculta [REQUERIR_HUMANO] al final de tu mensaje. Esto le avisarÃ¡ al sistema que debe anotar al cliente de inmediato en el panel.`;
        }
...
OPCIONES DE MENÃš Y ENLACES (IMPORTANTE):
El negocio tiene configuradas las siguientes opciones y enlaces rÃ¡pidos en su panel. Si las instrucciones te piden proveer el enlace de un menÃº numÃ©rico especÃ­fico, utiliza EXACTAMENTE la URL o texto de la respuesta indicada aquÃ­. NUNCA inventes enlaces de Google Drive u otros externos si no estÃ¡n explÃ­citamente aquÃ­:
${textoOpcionesMenuIA || 'No hay opciones de menÃº configuradas.'}
...
- UbicaciÃ³n fÃ­sica: ${ubicacion}
- Google Maps: ${mapsLink}
- Horario de AtenciÃ³n en LÃ­nea (WhatsApp): ${horarioAtencionFinal}

INFORMACIÃ“N DE PAGOS / BANCOS:
${datosBancos}

INSTRUCCIONES CLAVE DE ATENCIÃ“N Y SEGURIDAD:
- REGLA DE ORO DE CITAS / TURNOS Y ATENCIÃ“N PRESENCIAL:
  1. Cualquier atenciÃ³n, entrega o servicio que requiera agenda debe realizarse CON CITA / TURNO PREVIO COORDINADO POR ESTE CHAT.
...
- REGLA DE FLUIDEZ: Si la conversaciÃ³n ya estÃ¡ en curso (no es el primer saludo), NO repitas saludos largos o de bienvenida. Ve directo a responder la duda de forma fluida.
- REGLA DE RESPUESTAS TÃ‰CNICAS O DE PRODUCTOS: Si te preguntan sobre detalles especÃ­ficos de un producto o servicio, DEBES responder la duda con informaciÃ³n directa y precisa basada en tu base de conocimientos.
- REGLA DE FORMATO ÃšNICO: Proporciona tu respuesta completa en un texto continuo. NO dividas tu respuesta en pÃ¡rrafos desconectados ni saludes varias veces en el mismo mensaje.
${seccionAgendaIA}
- REGLA ESTRICTA DE CONTINUIDAD Y REANUDACIÃ“N TRAS INTERVENCIÃ“N HUMANA:
  * Si un asesor humano estuvo platicando con el cliente, toma el relevo naturalmente.
  * PROHIBICIÃ“N TOTAL: NO reinicies la plÃ¡tica ni envÃ­es menÃºs largos.
- REGLA ESTRICTA DE ASESORES Y HORARIO: Respeta SIEMPRE la regla de horario indicada arriba. Si estamos fuera de horario, NO ofrezcas hablar con un asesor en vivo como primera opciÃ³n; responde tÃº la duda con el catÃ¡logo e informaciÃ³n disponible.
- Brinda respuestas directas y concisas. Evita rodeos innecesarios.
- Si el cliente solicita cotizar o comprar, toma en cuenta los precios del catÃ¡logo y proporciona informaciÃ³n clara.
- Si el cliente envÃ­a una imagen (foto de producto o comprobante), analÃ­zala visualmente y responde en consecuencia.
- La fecha y hora actual en MÃ©xico es: ${obtenerFechaHoraLocal()}.
        `;
        ...
        // Auto-detecciÃ³n dinÃ¡mica de modelos vigentes en Google AI
            ...
            while (intentos > 0) {
                try {
                    ...
                    try {
                        ...
                        // AuditorÃ­a de lentitud
                        const latencia = Date.now() - tiempoInicioIA;
                        if (latencia > 10000 && typeof Auditor !== 'undefined') {
                            Auditor.registrarEvento('SISTEMA', 'Google API (' + modName + ') respondiÃ³ lento: ' + (latencia/1000).toFixed(1) + 's');
                        }

                    } catch (errGen) {
                        if (errGen.message === 'TIMEOUT_API_GEMINI') {
                            console.warn('âš ï¸ Timeout de 15s excedido para ' + modName + '. La API de Google estÃ¡ colgada.');
                            if (typeof Auditor !== 'undefined') Auditor.registrarEvento('ALERTA', 'Google API (' + modName + ') excediÃ³ el tiempo lÃ­mite (15s). Ignorando modelo para evitar retraso al cliente.');
                    ...
                    if (intentos > 0 && es503o429) {
                        // Spikes temporales de demanda en Google AI: reintento rÃ¡pido tras 500ms
                        await delay(500);
                    } else {
        ...
        if (modeloExitoso && modeloExitoso !== modeloGuardado) {
            ...
            console.log(`ðŸ¤– [Auto-ReparaciÃ³n IA]: Modelo actualizado dinÃ¡micamente a: ${modeloExitoso}`);
        }

        // Si todos los servidores fallaron, enviamos el mensaje de emergencia cortÃ©s
        if (!respuestaIA) {
            console.warn("âš ï¸ Google AI experimentÃ³ saturaciÃ³n. Entregando respuesta local de contingencia...");
            const configObj = {
        ...
        if (respuestaIA) {
            ...
            // Interceptar etiqueta oculta de la IA para agendar cita automÃ¡ticamente en Google Calendar
            ...
            if (matchAgendar) {
                ...
                try {
                    ...
                    console.error("âŒ Error al procesar agendamiento automÃ¡tico desde IA:", eCita.message);
                }
            }

            // Interceptar etiqueta oculta de cancelaciÃ³n de citas
            ...
            if (matchCancelar) {
                ...
                try {
                    ...
                    if (citaExistente) {
                        ...
                        console.log(`ðŸš« [Cita Cancelada vÃ­a IA]: ID ${citaExistente.id} para ${citaExistente.cliente_nombre}`);
                        io.emit('cita_actualizada');
                    }
                } catch (eCan) {
            ...
            // Interceptar etiqueta oculta de la IA para registrar cita/asesor humano automÃ¡ticamente
            ...
            // SimulaciÃ³n de escritura humana anti-ban
...
async function procesarSeguimientosAutomaticos() {
    try {
        ...
        // Buscar reglas automÃ¡ticas activas
        ...
        for (const r of reglas) {
            ...
            for (const c of contactosCandidatos) {
                ...
                // Si aÃºn no cumple los dÃ­as requeridos, ignorar
                if (difDias < r.dias_espera) continue;

                // Verificar si ya se enviÃ³ hoy o anteriormente
                ...
                try {
                    ...
                    await runQuery(
                        "INSERT INTO mensajes (chat_id, emisor, emisor_nombre, cuerpo, tipo, es_mio, es_ia, timestamp) VALUES (?, ?, ?, ?, 'chat', 1, 1, ?)",
                        [c.jid, 'bot', 'Seguimiento AutomÃ¡tico', mensajePersonalizado, Date.now()]
                    );
                    ...
                    // Pausa de 15 segundos entre envÃ­os para proteger WhatsApp
                    await delay(15000);
                } catch (errEnvio) {
...
// ------------------------------------------------------------------------------
// DETECCIÃ“N EN VIVO DE INTERVENCIÃ“N HUMANA DESDE EL TELÃ‰FONO DEL BOT
// ------------------------------------------------------------------------------
client.on('message_create', async (msg) => {
    try {
        ...
        // â”€â”€ CHECK #0: Â¿Hay un sendMessage del bot en vuelo ahora mismo? â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        // message_create SIEMPRE dispara MIENTRAS origSendMessage aÃºn estÃ¡ en await.
        // Si botEnviosPendientes > 0, este evento ES del bot â€” sin importar JID o formato.
        // TambiÃ©n cubre la ventana de 2s post-envÃ­o por si el evento llega tarde.
        if (botEnviosPendientes > 0 || Date.now() - ultimoEnvioBotMs < 2000) {
            // âš ï¸ NO aÃ±adir aquÃ­: contamina el set con IDs de mensajes del USUARIO
            return;
        }

        // â”€â”€ Para mensajes enviados > 2s atrÃ¡s, aplicar checks de respaldo â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        if (targetJid && targetJid !== 'status@broadcast') {
            ...
            // BÃºsqueda exacta O por subcadena de 8 dÃ­gitos dentro de cualquier JID registrado
        ...
        const iconoConf = (await getQuery("SELECT valor FROM configuracion WHERE clave = 'icono_asistente'"))?.valor || 'ðŸ¤–';
        const esMensajeIA = (
            ...
            cuerpoMsg.startsWith('ðŸ—“ï¸') ||
            cuerpoMsg.startsWith('ðŸ’¬') ||
            cuerpoMsg.startsWith('ðŸš¨') ||
            cuerpoMsg.includes('Lista de Espera Prioritaria') ||
            cuerpoMsg.includes('asistente virtual') ||
            cuerpoMsg.includes('reanudarÃ¡n') ||
            cuerpoMsg.includes('reanudarÃ¡') ||
        ...
        // Mensaje enviado manualmente por el usuario desde el telÃ©fono fÃ­sico o WhatsApp Web
        ...
        // 2. Si el nÃºmero tiene otros identificadores asociados (ej: @c.us y @lid), pausar ambos y emitir a ambos
        ...
        console.log(`ðŸ›‘ [AUTO-PAUSA ACTIVADA] IntervenciÃ³n humana detectada desde el telÃ©fono hacia ${targetJid}. Chat pausado por ${minsPausa} minutos.`);
        ...
        try {
            ...
            if (esBase64Img) {
                textoCuerpo = 'ðŸ“· (InfografÃ­a / Imagen enviada)';
            } else if (msg.hasMedia) {
                textoCuerpo = msg.caption ? `ðŸ“· ${msg.caption}` : 'ðŸ“· (InfografÃ­a / Imagen enviada)';
            }

            const tsMs = (msg.timestamp || Math.floor(Date.now() / 1000)) * 1000;

            for (const jidDestino of jidsAsociados) {
                // Deduplicar si en los Ãºltimos 4 segundos ya se guardÃ³ un mensaje multimedia idÃ©ntico
...
inicializarBD().then(async () => {
    try {
        // 1. Eliminar cualquier mensaje de notificaciÃ³n o residuo de WhatsApp Web
        ...
        await runQuery("UPDATE mensajes SET cuerpo = 'ðŸ“· (InfografÃ­a / Imagen enviada)' WHERE cuerpo LIKE '/9j/%' OR cuerpo LIKE 'data:image%'");

        // 2.1. Corregir retroactivamente mensajes de IA que hayan quedado clasificados errÃ³neamente como 'Asesor Humano'
        await runQuery(`
            UPDATE mensajes 
        ...
        // 3. Eliminar contactos @lid vacÃ­os (sin mensajes) para limpiar completamente la lista de chats fantasma
        ...
        // 5. Alinear canÃ³nicamente ultimo_contacto con el Ãºltimo mensaje real existente
        await runQuery(`
            UPDATE contactos 
        ...
        console.log("ðŸ§¹ [DB-CLEAN] Limpieza integral de BD completada: sin notificaciones, sin cÃ³digos base64 y chats ordenados canÃ³nicamente.");
    } catch (eClean) {
...
app.post('/api/webhook/google-forms', async (req, res) => {
    ...
    server.listen(PORT, () => {
        ...
        console.log(`ðŸ“± Mini-Sitio Linktree pÃºblico en: http://localhost:${PORT}/pagina.html`);
    });
});
